...
class traeger:
            ...
            while self.mqtt_thread_running:
                try:
                    ...
                    self.mqtt_client.loop_forever(
                        timeout=1.0, retry_first_connection=True
                    )
    ...
    async def get_mqtt_client(self):
        ...
        try:
            ...
            _LOGGER.info(
                f"Attempting MQTT connection (Thread count: {threading.active_count()})"
            )
    ...
    def mqtt_onconnectfail(self, client, userdata):
        ...
        if self.consecutive_failures >= self.max_retry_attempts:
            ...
            retry_delay = min(
                self.base_retry_delay * (2**self.consecutive_failures),
                self.max_retry_delay,
            )
            # Add jitter to prevent thundering herd
            retry_delay += random.uniform(0, retry_delay * 0.1)
            _LOGGER.warning(
                f"MQTT connection failed (attempt {self.consecutive_failures}/{self.max_retry_attempts}). Retrying in {retry_delay:.1f} seconds"
            )
    ...
    def mqtt_ondisconnect(self, client, userdata, rc):
        ...
        if rc != 0:  # Unexpected disconnection
            _LOGGER.warning(
                f"Unexpected MQTT disconnection (rc={rc}). Will attempt reconnection."
            )
    ...
    async def main(self):
        ...
        connection_age = (
            time.time() - self.last_successful_connection
        ...
        if self.mqtt_thread_running:
            ...
            try:
                for grill in self.grills:
                    grill_id = grill["thingName"]
                    if (
                        grill_id in self.grill_status
                        and "status" in self.grill_status[grill_id]
                    ):
                        ...
                        if grill_id in self.grill_callbacks:
                            for callback in self.grill_callbacks[grill_id]:
                                try:
                                    ...
                                    _LOGGER.error(
                                        f"Error in callback for grill {grill_id}: {e}"
                                    )
            except Exception as e:
    ...
    async def _attempt_reconnect(self):
        ...
        try:
            _LOGGER.info(
                f"Attempting MQTT reconnection (attempt {self.consecutive_failures + 1})"
            )
            self.mqtt_thread_refreshing = True
